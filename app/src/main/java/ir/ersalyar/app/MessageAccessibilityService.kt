...
class MessageAccessibilityService : AccessibilityService() {
    ...
    private fun handleCommand(intent: Intent) {
        ...
        val groups =
            if (p.groupNames.isEmpty()) listOf(p.groupName) else p.groupNames
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (scanChannel != null && event?.packageName != packageName) {
            handler.postDelayed({ scanCurrentWindow() }, 350)
        }
    }
    ...
    private fun scanCurrentWindow() {
        ...
        val action =
            if (scanContacts) ACTION_CONTACTS_SCANNED else ACTION_GROUPS_SCANNED
        ...
        sendBroadcast(
            Intent(action)
                ...
                .putStringArrayListExtra(
                    if (scanContacts) "contacts" else "groups",
                    ArrayList(cleaned)
                )
        )
    ...
    private fun collectCandidateTexts(
    ...
    private fun trySend() {
        ...
        val groupNode = findText(root, p.target) ?: run {
            ...
            if (p.attempts >= 8) {
                finishCurrent(
                    false,
                    "مخاطب/گروه پیدا نشد پس از چند تلاش"
                )
                return
            }
        ...
        handler.postDelayed({
            ...
            handler.postDelayed({
                ...
                if (send != null) {
                    if (
                        p.imageUris.isNotEmpty() ||
                        !p.imageUri.isNullOrBlank()
                    ) {
                        runCatching {
                            val uris =
                                if (p.imageUris.isNotEmpty()) {
    ...
    private fun findSendButton(
        ...
        for (name in names) {
            node.findAccessibilityNodeInfosByText(name)
                .firstOrNull()
                ?.let {
                    return it
                }
